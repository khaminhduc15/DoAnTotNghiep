...
function showAllSong(){
    document.getElementById('back-user').style.display = "";
    axios.get(url).then(res => {
        // console.log("data", res.data);
        let str = ` <table class="all-song-tb" style="margin-top: 100px">
        ...
        str += `</table>`;
        document.getElementById('back-user').innerHTML = str;
    })
}
function TopNew(){
...
function playSong(indexSong) {
    ...
    axios.get(`http://localhost:8080/api/songs/listens/${song.id}`
    ).then(res => {
        console.log(res.data)
...
function searchSong2() {
    let search = document.getElementById('searchSong').value;
    if (search !== null) {
        dataSong2(`http://localhost:8080/api/songs?name=${search}`)
    }
}
function dataSong2(url) {
    axios.get(url).then(res => {
...
function showAllAlbum(){
    document.getElementById('choice-playlist2').style.display = 'none';
    document.getElementById('choice-new').style.display = 'none';
    document.getElementById('choice-album').style.display = 'none';
    document.getElementById('playlist-selected').style.display = 'block';

    axios.get(`http://localhost:8080/api/albums`).then(res => {
        let dataAlbum = res.data;
        ...
        res.data.forEach((item, index) => {
            str += `<tr class="all-song-tbody" onclick="showAlbumSong(${item.id})">
                                <td><img src="${item.avatar}" style="width: 50px;height: 50px;margin-top: 10px;margin-left: 30px"></td>
                                <td>${item.name}</td>
...
function showAllSongAuthor(){
    document.getElementById('label_showAllSong').style.display='none';
    axios.get(`http://localhost:8080/api/songs/${currentId}`).then(res => {
        let data = res.data;
        console.log(data);
        let str = ` <table class="all-song-tb" style="margin-top: 100px">
                            <tr class="all-song-thead">
        ...
        res.data.forEach((item, index) => {
            str += `<tr class="all-song-tbody">
                                ...
                                <td>${item.album.name}</td>
                                <td>${item.category.name}</td>
                                <td><button onclick="showEditSongForm(${item.id})">Edit</button></td>
                                <td><button onclick="removeSong(${item.id})">Delete</button></td>
                            </tr>`
        })
        str += `</table>`;
        document.getElementById('author-song-item').innerHTML = str;
